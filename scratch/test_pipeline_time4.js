...
async function test() {
  ...
  async function computeHsvHistogram(imagePath) {
    try {
  ...
  for (const [id1Str, targetSet] of candidateMap.entries()) {
    ...
    for (const id2 of targetSet) {
      ...
      // If color histogram is evaluated:
      if (dHashDist <= 36 && !(dHashDist <= 24 && pHashDist <= 10)) {
        await Promise.all([
          computeHsvHistogram(r1.filePath),
          computeHsvHistogram(r2.filePath)
        ]);
      }
      pairsChecked++;
    }
  ...
  console.log(`Evaluated ${pairsChecked} candidate pairs in:`, Date.now() - tStart, 'ms');
}
test();
