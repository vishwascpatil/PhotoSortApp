...
function classifyText(text) {
  ...
  for (const rule of ocrRules) {
    ...
    // STRICT: MUST match at least one required keyword
...
async function main() {
  ...
  const res = db.exec("SELECT id, filename, is_document, document_category, extracted_text FROM photos WHERE extracted_text != '' AND extracted_text != 'NONE' AND extracted_text != 'ERROR'");
  ...
  for (const r of rows) {
    ...
    if (result.isDoc) {
      docs.push({ id, filename, type: result.classification, cat: result.category, score: result.score });
    } else {
      nonDocs.push({ id, filename, reason: result.reason || 'No rule match' });
    }
  }

  console.log(`\n=== REAL DOCUMENTS FOUND (${docs.length}) ===`);
  for (const d of docs) {
    console.log(`[DOC] ${d.filename} -> "${d.type}" | Category: "${d.cat}" (Score: ${d.score})`);
  }

  console.log(`\n=== REJECTED NON-DOCUMENTS (${nonDocs.length}) ===`);
  for (const nd of nonDocs) {
    console.log(`[REJECT] ${nd.filename} -> ${nd.reason}`);
  }
}
