...
function classifyExtractedText(extractedText) {
  ...
  if (memeDetection.isMeme) {
    return { isDocument: false, classification: 'not_a_document', category: null, confidence: 0, reason: 'Meme/Chat disqualified' };
  }
...
async function run() {
  ...
  console.log('Testing exact classifyExtractedText on all 83 photos with text...');
  const docs = [];
  const nonDocs = [];
  ...
  console.log(`\nDOCUMENTS CLASSIFIED (${docs.length}):`);
  console.table(docs);

  console.log(`\nCHECK FALSE POSITIVES IN NON-DOCS:`);
  console.log('PSNS9945.JPG in non-docs?', nonDocs.some(n => n.filename === 'PSNS9945.JPG'));
  console.log('FZQB2179.JPG in non-docs?', nonDocs.some(n => n.filename === 'FZQB2179.JPG'));
}
