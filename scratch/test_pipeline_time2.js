...
async function test() {
  ...
  let fullShaNeeded = 0;
  let colorHistNeeded = 0;
  let rotVariantsNeeded = 0;
  ...
  for (const [id1Str, targetSet] of candidateMap.entries()) {
    ...
    for (const id2 of targetSet) {
      ...
      if (r1.fileSize && r2.fileSize && r1.fileSize === r2.fileSize && r1.fileSize > 0) {
        const p1 = r1.partialSha256;
        const p2 = r2.partialSha256;
        if (p1 && p2 && p1 === p2) {
          if (!r1.sha256 || !r2.sha256) fullShaNeeded++;
        }
      }
  ...
  console.log('Results:', {
    candidateMapEntries: candidateMap.size,
    fullShaNeeded,
    rotVariantsNeeded,
    colorHistNeeded
  });
